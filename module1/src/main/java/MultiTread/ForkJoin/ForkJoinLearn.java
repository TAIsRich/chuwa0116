...
public class ForkJoinLearn {
    @Test
    public void demo01(){
        ForkJoinTask forjoinTask = new ForkJoinTask(1, 10000);
        ForkJoinPool forkJoinPool = new ForkJoinPool();
        Future<Integer> future = forkJoinPool.submit(forjoinTask);
        try {
...
class ForkJoinTask extends RecursiveTask<Integer>{
    ...
    protected Integer compute() {
        ...
        if(last - first <= threhold){
            //small task cal the result directly
