...
- Global Exception Handler: we can use `@ControllerAdvice` annotation with any class to define our global exception handler. The handler methods in Global Controller Advice is same as Controller based exception handler methods and used when controller class is not able to handle the exception.

- HandlerExceptionResolver: we can implement `HandlerExceptionResolver` interface to create global exception handler.

## 4.  How do you do the validations in Spring?
- Import `javax.validation` and `hibernate-validator` dependencies to the project
- Add validation Rule to `payload`
- Add `@valid` annotation to controller to apply the Rule
...
- Spring 简介： https://www.youtube.com/watch?v=l0MqsOADAUE&list=PLmOn9nNkQxJFbsU4Qz8CdRiVM4Qs3ci75&index=64
...
- DI: i.  https://www.youtube.com/watch?v=MgTpBST9onM&list=PLmOn9nNkQxJFbsU4Qz8CdRiVM4Qs3ci75&index=70 
