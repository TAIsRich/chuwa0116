...
public class StreamApiTest {
	...
	public void exercise1() {
		...
		List<Product> result = productRepo.findAll().stream().filter(p -> (p.getCategory().equalsIgnoreCase("Books") && p.getPrice() > 100)).collect(Collectors.toList());
	...
	public void exercise1a() {
		...
		Predicate<Product> isBook = p -> p.getCategory().equalsIgnoreCase("Book");
		Predicate<Product> expensiveThanOneHundred = p -> p.getPrice() > 100;
		Predicate<Product> isBookAndExpensive = isBook.and(expensiveThanOneHundred);

		List<Product> result = productRepo.findAll().stream().filter(isBookAndExpensive).collect(Collectors.toList());
	...
	public void exercise1b() {
		...
		long startTime = System.currentTimeMillis();
		BiPredicate<String, Double > filter = (c, p)-> c.equals("Book") && p > 100;
		List<Product> result = productRepo.findAll().stream().filter(p -> filter.test(p.getCategory(), p.getPrice())).collect(Collectors.toList());
		long endTime = System.currentTimeMillis();
	...
	public void exercise2() {
		...
		List<Order> result = orderRepo.findAll().stream().filter(o -> o.getProducts().stream().anyMatch(p -> p.getCategory().equalsIgnoreCase("Book"))).collect(Collectors.toList());
	...
	public void exercise3() {
		...
		double discount = 0.9;
		List<Product> result = productRepo.findAll().stream().filter(p -> p.getCategory().equalsIgnoreCase("Toys")).map(p -> p.withPrice(p.getPrice() * discount)).collect(Collectors.toList());
	...
	public void exercise4() {
		...
		List<Product> result = orderRepo.findAll().stream().filter(p-> p.getCustomer().getTier().equals(2) && p.getOrderDate().compareTo(LocalDate.parse("01-Feb-2021", formatter)) > 0 && p.getOrderDate().compareTo(LocalDate.parse("01-Apr-2021", formatter)) < 0).flatMap(p -> p.getProducts().stream()).collect(Collectors.toList());
...
log.info(result.get().toString());
	...
	public void exercise6() {
		...
		List<Order> result = orderRepo.findAll().stream().sorted(Comparator.comparing(Order::getOrderDate)).limit(3).collect(Collectors.toList());
	...
	public void exercise7() {
		...
		List<Product> result = orderRepo.findAll().stream().filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 15))).flatMap(o -> o.getProducts().stream()).distinct().collect(Collectors.toList());
	...
	public void exercise8() {
		long startTime = System.currentTimeMillis();
		double result = orderRepo.findAll().stream().filter(o -> o.getOrderDate().compareTo(LocalDate.of(2021, 2, 1)) >= 0 && o.getOrderDate().compareTo(LocalDate.of(2021, 3, 1)) < 0).flatMap(o -> o.getProducts().stream()).map(p -> p.getPrice()).reduce(0.0, (p1, p2)->p1+p2);
	...
	public void exercise8a() {
		...
		double result = (double) orderRepo.findAll().stream()
				.filter(o -> o.getOrderDate().compareTo(LocalDate.of(2021, 2, 1)) >= 0 && o.getOrderDate().compareTo(LocalDate.of(2021, 3, 1)) < 0)
				.flatMap(o -> o.getProducts().stream()).reduce(0D, accumulator, Double::sum);
	...
	public void exercise9() {
		...
		double result = orderRepo.findAll().stream()
				.filter(o -> o.getOrderDate().equals(LocalDate.of(2022, 3, 15)))
	...
	public void exercise11() {
		long startTime = System.currentTimeMillis();
		Map<Long, Integer>  result =  orderRepo.findAll().stream().collect(Collectors.toMap(Order::getId, order->order.getProducts().size()));
	...
	public void exercise12a() {
		...
		Map<Long, List<Long>> result =  orderRepo.findAll().stream()
				.collect(Collectors.groupingBy(o->o.getCustomer().getId(), Collectors.mapping(Order::getId, Collectors.toList())));
		long endTime = System.currentTimeMillis();
	...
	void exercise15a() {
		...
		Map<String, String> result = productRepo.findAll().stream()
				.collect(Collectors.groupingBy(Product::getCategory, Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(Product::getPrice)), p -> p.map(Product::getName).orElse(null))));
