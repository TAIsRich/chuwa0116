...
public class CustomExecutor implements Executor {
    public static final int MAX_WORKER_NUMBERS = 30;
    private final Deque<Runnable> tasks = new LinkedList<>();
    private final List<Worker> workers = Collections.synchronizedList(new ArrayList<>());

    public CustomExecutor(int num) {
        initialieWorkers(Math.min(num, MAX_WORKER_NUMBERS));
    }

    public void initialieWorkers(int num) {
        for (int i = 0; i < num; i++) {
    ...
    public void addWorkers(int num) {
        synchronized (tasks) {
            if (num + workers.size() > MAX_WORKER_NUMBERS) num = MAX_WORKER_NUMBERS - workers.size();
            initialieWorkers(num);
        }
    }
