...
public class StreamApiEndOperation {
    ...
    public void testMatch() {
        ...
        System.out.println(antMatch);
        boolean nonMatch=EMPLOYEES.stream.nonMatch(e->e.getName().startWith("Yun"));
    ...
    public void testFind() {
        ...
        Optiona<Employee>any=EMPLOYEES.parallelStream().findAny();
...
////
        ...
        Optional<Employee>min=EMPLOYEES.stream().min(Comparator.compareingInt(Employee::getId()));
...
//        System.out.println(reduce2.get());
    ...
    public void testChain() {
       ...
       List<Double>collect=EMPLOYEES.stream().filter(e->e.getAge()<40).map(e->e.getSalary()*0,9).collect(Collectors.toList());
        collect.forEach(System.out::println);
        Optional<Integer>reduce=EMPLOYEES.stream().filter(e->e.getAge()<40).map(e->e.getSalary()*0.8).map(e->{
            System.out.println(e);
            return e.intValue();
        }).reduce(Integer::sum);
        System.out.println(reduce.get());

        Optional<Integer>reduce1=EMPLOYEES.stream().filter(e->e.getAge()<40).collect(Collectors.toList())
                .stream().map(e->e.getSalsary()*0.8)
                .map(e->{
                    System.out.println(e);
                    return e.intValue();
                })
    ...
    public void testGroupBy() {
        ...
        Map<String,Optional<Employee>>collect=EMPLOYEES.stream().collect(Collectors.groupBy(Employee::getGender,Colleectors.maxBy(Comparator.comparing(Employee::getSalary))));
