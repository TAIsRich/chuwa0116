...
public class StreamApiIntermediateOperation {
    ...
    public void testFlatMap() {
        ...
        ListOfStrings.add(lists1);
        ListOfStrings.add(list2);
        System.out.println("listsOfStrings: " + listsOfStrings);
        System.out.println("**** 分步骤使用flatmap找到最大值 *****");
        StreamL<String>stringStream=listOfStrings.stream().flatMap(list->list.stream());
        Stream<Stream<String>>streamStream=listOfStrings.stream.map(list->list.stream());
        ...
        int asInt1=listOfStrings.stream().flatMap(list->list.stream()).mapToInt(Integer::parseInt).max().getAsInt();
